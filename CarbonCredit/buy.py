...
def load_contract():
    ...
    with open(Path('./contracts/compiled/deployer_abi.json')) as p:
        contract_abi = json.load(p)

    # Set the contract address (this is the address of the deployed contract)
    contract_address = os.getenv("CROWDSALE_SMART_CONTRACT")
...
st.title("Carbon Credit Buying System")
...
st.markdown("## Carbon Credit : CCGA")
st.markdown("### Calculator")

amount_in_Ether = st.number_input("Enter Purchase Amount in Ether")
...
# amount_in_USD = amount_in_Wei * conversion
# amount = amount_in_Wei - weiValue_brokerage

token_price_in_Wei = 5

Wei_Rate = 0.000000000000000001*1,629.74

brokerage_fee = 0.02

brokerage_amount = amount_in_Wei * brokerage_fee

brokerage_amount_wei = brokerage_amount/1000000000000000000 

amount = amount_in_Wei - brokerage_amount

total_token = amount/token_price_in_Wei

token_USD = amount_in_Ether*1,629.74

if st.button("Calculate"):
    st.write("the brokerage fee is 2%")
    st.write("the price for credit is Wei",token_price_in_Wei)
    st.write("the total number of carbon credits purchased is", total_token) 
    st.write("the cost in USD is ",token_USD)
    st.write("the brokerage fee is", brokerage_amount_wei)
    st.write("All transactions are exclusive of gas fee")
    st.write(amount)
st.markdown("---")

if st.button("Buy CCGA Token"):
    tx_hash = contract.functions.buyTokens(
        address,
    ).transact({'from': address,'value': int(amount) ,'gas': 1000000})
    receipt = w3.eth.waitForTransactionReceipt(tx_hash)
    st.write("Your Transaction Receipt is:")
    st.write(dict(receipt))
st.markdown("---")

# if st.button("BuyTokens"):
#     tx_hash = contract.functions.token(
#         address,
#         amount,
#         project_uri
